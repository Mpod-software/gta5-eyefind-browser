...
(function () {
  ...
  /* ================================================================== *
   * History flyout — persisted to localStorage so it survives restarts.
   * ================================================================== */
  const STORAGE_KEY = 'eyefind.history';
  const MAX_HISTORY = 80;

  // Read the saved list, tolerating missing, corrupt, or legacy data.
  function loadHistory() {
    try {
      ...
      return parsed
        .filter(function (e) { return e && typeof e.url === 'string'; })
        .slice(0, MAX_HISTORY);
    } catch (_) {
      return [];
    }
  ...
  // Persist the current list; never let a quota/serialization error surface.
  ...
  function renderHistory() {
    ...
    if (historyClear) historyClear.disabled = history.length === 0;
  ...
  // Wipe the tracked history from both memory and localStorage, then instantly
  ...
  if (historyClear) {
    historyClear.addEventListener('click', function (event) {
      event.stopPropagation(); // keep the flyout open on the fresh empty state
      Sound.play('click');
      clearHistory();
    });
  }
